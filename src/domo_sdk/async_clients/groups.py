...
class AsyncGroupClient(AsyncDomoAPIClient):
    ...
    async def create(self, group_request: dict) -> dict:
        """Create a new group."""
        return await self._create(URL_BASE, group_request)

    async def get(self, group_id: int) -> dict:
        """Retrieve a single group by ID."""
        return await self._get(f"{URL_BASE}/{group_id}")
    ...
    async def update(self, group_id: int, group_update: dict) -> dict:
        """Update an existing group."""
        return await self._update(f"{URL_BASE}/{group_id}", group_update)
    ...
    async def list_users(self, group_id: int, limit: int = 50, offset: int = 0) -> list:
