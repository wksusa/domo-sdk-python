"""Async Search client for the Domo API."""
...
class AsyncSearchClient(AsyncDomoAPIClient):
    """Search across Domo objects asynchronously.
    ...
    async def search_datasets(
        ...
        if auth_mode == "developer_token":
            return await self._search_datasets_dev_token(query, count, offset)
        return await self._search_datasets_oauth(query, count, offset)

    # ------------------------------------------------------------------
    # Private helpers
    # ------------------------------------------------------------------

    async def _search_datasets_dev_token(
        self,
        query: str,
        count: int,
        offset: int,
        ...
        result = await self._create("/data/ui/v3/datasources/search", payload)
        if isinstance(result, dict):
            return result.get("dataSources", [])
        return result if isinstance(result, list) else []

    async def _search_datasets_oauth(
        self,
        query: str,
        count: int,
        offset: int,
        ...
        result = await self._list("/v1/datasets", params=params)
